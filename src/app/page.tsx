...
export default function Home() {
  ...
  async function handlePhoneSubmit(e: React.FormEvent) {
    ...
    if (digits.length < 10) {
      setError('Coloca o telefone completo com DDD.')
      return
    }
    setError('')
    setLoading(true)
  ...
  async function handleNameSubmit(e: React.FormEvent) {
    ...
    if (name.trim().length < 2) {
      setError('Coloca seu nome completo.')
      return
    }
    setError('')
    setLoading(true)
    ...
    const { data, error: dbError } = await supabase
      .from('customers')
      .insert({ phone: digits, name: name.trim(), points: 0 })
      .select('id, name')
      .single()
    ...
    if (dbError || !data) {
      setError('Erro ao cadastrar. Tenta de novo.')
      return
    }
    ...
    <main className="min-h-screen bg-brand-red flex flex-col">
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 pt-12 pb-6">
        ...
        <div className="text-center mb-10">
          <div className="inline-flex items-center justify-center w-20 h-20 rounded-full bg-brand-gold mb-4 shadow-lg">
            <span className="text-4xl font-display text-brand-dark leading-none">AH</span>
          </div>
          ...
          <p className="text-white/80 text-sm font-body mt-1 tracking-wide uppercase">
            Depósito e Mercearia · Califórnia
          </p>
        </div>
        ...
        <div className="w-full max-w-sm bg-brand-dark rounded-2xl p-6 shadow-2xl">
          {step === 'phone' ? (
            <>
              <h2 className="font-display text-brand-gold text-3xl mb-1">
                SEU NÚMERO
              </h2>
              <p className="text-white/60 text-sm font-body mb-5">
                Coloca o telefone pra acessar seus pontos.
              </p>
              <form onSubmit={handlePhoneSubmit} className="space-y-4">
                ...
                />
                {error && <p className="text-brand-gold text-sm font-body">{error}</p>}
                <button
                  type="submit"
                  disabled={loading}
            ...
            <>
              ...
              <p className="text-white/60 text-sm font-body mb-5">
                Primeira vez aqui? Bem-vindo! Qual é o seu nome?
              </p>
      ...
      <div className="text-center pb-8">
        <p className="text-white/40 text-xs font-body">
          R$1 comprado = 1 ponto. Simples assim.
        </p>
      </div>
    </main>
  )
